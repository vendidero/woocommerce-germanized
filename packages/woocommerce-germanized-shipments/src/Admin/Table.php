...
class Table extends WP_List_Table {
	...
	protected function get_views() {
		...
		foreach ( wc_gzd_get_shipment_statuses() as $status => $title ) {
			...
			$status_label = sprintf(
				translate_nooped_plural( _nx_noop( ( $title . ' <span class="count">(%s)</span>' ), ( $title . ' <span class="count">(%s)</span>' ), 'shipments', 'woocommerce-germanized' ), $num_shipments[ $status ] ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralSingular, WordPress.WP.I18n.NonSingularStringLiteralPlural
	...
	public function column_title( $shipment ) {
		...
		echo '<a href="#" class="shipment-preview has-shipment-modal" data-id="wc-gzd-modal-preview-shipment" data-load-async="1" data-reference="' . esc_attr( $shipment->get_id() ) . '" data-nonce-params="wc_gzd_shipments_admin_shipments_table_params">' . esc_html_x( 'Preview', 'shipments', 'woocommerce-germanized' ) . '</a>';
				...
				<div class="wc-backbone-modal-content">
					<section class="wc-backbone-modal-main" role="main">
						<header class="wc-backbone-modal-header">
							<h1><?php printf( esc_html_x( '%1$s %2$s', 'shipments-preview-title', 'woocommerce-germanized' ), esc_html( wc_gzd_get_shipment_label_title( $shipment->get_type() ) ), esc_html( $shipment->get_shipment_number() ) ); ?></h1>
