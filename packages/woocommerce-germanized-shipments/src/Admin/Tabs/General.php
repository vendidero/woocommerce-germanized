...
class General extends Tab {
	...
	public function get_description() {
		return _x( 'Configure when and how to create shipments and manage information on your business.', 'shipments', 'woocommerce-germanized' );
	}
	...
	protected function get_automation_settings() {
		...
		$settings = array(
			...
			array(
				...
				'desc'    => _x( 'Mark order as completed after order is fully shipped.', 'shipments', 'woocommerce-germanized' ) . '<div class="wc-gzd-shipments-additional-desc">' . _x( 'This option will automatically update the order status to completed as soon as all required shipments have been marked as shipped.', 'shipments', 'woocommerce-germanized' ) . '</div>',
	...
	protected function get_return_settings() {
		$settings = array(
			array(
				...
				'desc'  => sprintf( _x( 'Returns can be added manually by the shop manager or by the customer. Decide what suits you best by turning customer-added returns on or off in your %s.', 'shipments', 'woocommerce-germanized' ), '<a href="' . esc_url( Settings::get_settings_url( 'shipping_provider' ) ) . '">' . _x( 'shipping provider settings', 'shipments', 'woocommerce-germanized' ) . '</a>' ),
			),
