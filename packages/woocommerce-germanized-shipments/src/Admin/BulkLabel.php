					...
					if ( $shipment->supports_label() ) {
						if ( $shipment->needs_label() ) {
							...
							if ( is_wp_error( $result ) ) {
								$this->add_notice( sprintf( _x( 'Error while creating label for %1$s: %2$s', 'shipments', 'woocommerce-germanized' ), '<a href="' . esc_url( $shipment->get_edit_shipment_url() ) . '" target="_blank">' . sprintf( _x( 'shipment #%d', 'shipments', 'woocommerce-germanized' ), $shipment_id ) . '</a>', $result->get_error_message() ), 'error' );
							} else {
