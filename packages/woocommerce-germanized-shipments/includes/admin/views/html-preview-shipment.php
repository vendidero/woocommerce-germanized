...
<div class="wc-gzd-preview-shipment">
	<p class="shipment-summary">
		<?php printf( esc_html_x( '%1$s %2$d/%3$d to ', 'shipments', 'woocommerce-germanized' ), esc_html( wc_gzd_get_shipment_label_title( $shipment->get_type() ) ), esc_html( $position_number ), esc_html( $total_shipments ) ); ?>
	...
	<div class="wc-gzd-preview-shipment-data columns">
		<div class="column col-6">
			...
			if ( ! empty( $provider ) && ! empty( $tracking_id ) ) :
				?>
				<p>
					<span class="shipment-shipping-provider"><?php printf( esc_html_x( 'via %s', 'shipments', 'woocommerce-germanized' ), wp_kses_post( wc_gzd_get_shipping_provider_title( $provider ) ) ); ?></span>
