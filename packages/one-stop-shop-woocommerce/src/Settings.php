...
class Settings {
	...
	protected static function get_oss_switch_html() {
		...
		?>
		<p>
			<a class="button button-secondary" onclick="return confirm('<?php echo esc_html( _x( 'Are you sure? Please backup your tax rates before proceeding.', 'oss', 'woocommerce-germanized' ) ); ?>');" href="<?php echo esc_url( self::get_oss_switch_link() ); ?>"><?php echo ( Helper::oss_procedure_is_enabled() ? esc_html_x( 'End OSS participation', 'oss', 'woocommerce-germanized' ) : esc_html_x( 'Start OSS participation', 'oss', 'woocommerce-germanized' ) ); ?></a>
			<a class="oss-settings-refresh-tax-rates" onclick="return confirm('<?php echo esc_html( _x( 'Are you sure? Please backup your tax rates before proceeding.', 'oss', 'woocommerce-germanized' ) ); ?>');" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=wc-status&tab=tools&action=refresh_oss_tax_rates' ), 'debug_action' ) ); ?> "><?php echo esc_html_x( 'refresh VAT rates', 'oss', 'woocommerce-germanized' ); ?></a>
			<a class="oss-settings-learn-more" href="https://vendidero.github.io/one-stop-shop-woocommerce/tax-adjustments"><?php echo esc_html_x( 'learn more', 'oss', 'woocommerce-germanized' ); ?></a>
		</p>
			<p class="oss-woocommerce-additional-desc wc-gzd-additional-desc"><?php echo esc_html_x( 'Use this option to automatically adjust tax-related options in WooCommerce. Warning: This option will delete your current tax rates and add new tax rates based on your OSS participation status.', 'oss', 'woocommerce-germanized' ); ?></p>
		<?php
	...
	protected static function get_observer_report_html() {
		...
		if ( ! $observer_report || Queue::get_running_observer() ) {
			...
			$status_link = $running ? '<a href="' . esc_url( $running->get_url() ) . '">' . esc_html_x( 'See status', 'oss', 'woocommerce-germanized' ) . '</a>' : '<a href="' . esc_url( add_query_arg( array( 'action' => 'oss_init_observer' ), wp_nonce_url( admin_url( 'admin-post.php' ), 'oss_init_observer' ) ) ) . '">' . esc_html_x( 'Start initial report', 'oss', 'woocommerce-germanized' ) . '</a>';
			$status_text = sprintf( ( $running ? esc_html_x( 'Report not yet completed. %s', 'oss', 'woocommerce-germanized' ) : esc_html_x( 'Report not yet started. %s', 'oss', 'woocommerce-germanized' ) ), $status_link );
			ob_start();
			?>
		...
		ob_start();
		?>
			<p class="oss-observer-details"><span class="oss-observer-total <?php echo esc_attr( $total_class ); ?>"><?php echo wc_price( $observer_report->get_net_total() ); ?></span> <?php echo esc_html_x( 'of', 'oss-amounts', 'woocommerce-germanized' ); ?> <span class="oss-observer-delivery-threshold"><?php echo wc_price( Package::get_delivery_threshold() ); ?></span> <span class="oss-observer-date-end"><?php printf( esc_html_x( 'As of: %s', 'oss', 'woocommerce-germanized' ), wc_format_datetime( $observer_report->get_date_end() ) ); ?></span> <a class="oss-settings-learn-more" href="<?php echo esc_url( $observer_report->get_url() ); ?>"><?php echo esc_html_x( 'see details', 'oss', 'woocommerce-germanized' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a></p>
