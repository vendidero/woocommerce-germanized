...
class Admin {
	...
	protected static function render_create_report() {
		...
		<div class="wrap oss-reports create-oss-reports">
			<form class="create-oss-report" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				<header>
					<h2><?php echo esc_html_x( 'New Report', 'oss', 'woocommerce-germanized' ); ?></h2>
				</header>
				<section>
	...
	protected static function render_reports() {
		...
		<div class="wrap oss-reports">
			<h1 class="wp-heading-inline"><?php echo esc_html_x( 'One Stop Shop', 'oss', 'woocommerce-germanized' ); ?></h1>
			<a href="<?php echo esc_url( add_query_arg( array( 'new' => 'yes' ), admin_url( 'admin.php?page=oss-reports' ) ) ); ?>" class="page-title-action"><?php echo esc_html_x( 'New report', 'oss', 'woocommerce-germanized' ); ?></a>
	...
	public static function render_report_details() {
		...
		<div class="wrap oss-reports oss-report-<?php echo esc_attr( $report->get_id() ); ?>">
			...
			<?php if ( 'completed' === $report->get_status() ) : ?>
				...
				?>
				<p class="summary"><?php printf( _x( 'Currently processed %1$s orders. Next iteration is scheduled for %2$s. <a href="%3$s">Find pending actions</a>', 'oss', 'woocommerce-germanized' ), esc_html( $details['order_count'] ), ( $details['next_date'] ? esc_html( $details['next_date']->date_i18n( wc_date_format() . ' @ ' . wc_time_format() ) ) : esc_html_x( 'Not yet known', 'oss', 'woocommerce-germanized' ) ), esc_url( $details['link'] ) ); ?></p>
			<?php endif; ?>
		</div>
		<?php
	}
