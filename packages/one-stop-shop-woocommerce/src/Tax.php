...
class Tax {
	...
	public static function get_tax_class_slugs() {
		...
		if ( false === $slugs ) {
			...
			foreach ( $tax_classes as $slug ) {
				...
				} elseif ( ! $reduced_tax_class && strstr( $slug, sanitize_title( __( 'Reduced rate', 'woocommerce' ) ) ) ) { // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
					$reduced_tax_class = $slug;
	...
	public static function import_rates( $rates, $tax_class = '' ) {
		...
		foreach ( \WC_Tax::get_rates_for_tax_class( $tax_class ) as $rate_id => $rate ) {
			if ( in_array( $rate->tax_rate_country, $eu_countries, true ) || self::tax_rate_is_northern_ireland( $rate ) || ( 'GB' === $rate->tax_rate_country && 'GB' !== Package::get_base_country() ) ) {
