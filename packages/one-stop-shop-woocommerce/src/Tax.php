	...
	public static function import_rates( $rates, $tax_class = '' ) {
		...
		foreach ( $rates as $iso => $rate ) {
			$_tax_rate = array(
				...
				'tax_rate_name'     => apply_filters( 'oss_import_tax_rate_name', sprintf( _x( 'VAT %s', 'oss-tax-rate-import', 'woocommerce-germanized' ), ( $iso . ( ! empty( $tax_class ) ? ' ' . $tax_class : '' ) ) ), $rate, $iso, $tax_class ),
				'tax_rate_priority' => 1,
				'tax_rate_compound' => 0,
