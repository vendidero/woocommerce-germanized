...
function eu_owb_get_order_withdrawal_full_name( $order, $id = '', $placeholder = false ) {
	...
	$full_name = $placeholder ? ( is_bool( $placeholder ) ? _x( 'Not specified', 'owb-full-name-placeholder', 'woocommerce-germanized' ) : $placeholder ) : '';
	...
	if ( $withdrawal = eu_owb_get_withdrawal_or_request( $order, $id ) ) {
		...
		if ( empty( $meta['last_name'] ) && ! empty( $meta['first_name'] ) ) {
			...
			$full_name = sprintf( _x( '%1$s', 'owb-first-name', 'woocommerce-germanized' ), $meta['first_name'] ); // phpcs:ignore WordPress.WP.I18n.NoEmptyStrings
		} elseif ( empty( $meta['first_name'] ) && ! empty( $meta['last_name'] ) ) {
			/* translators: 1: last name */
			$full_name = sprintf( _x( '%1$s', 'owb-last-name', 'woocommerce-germanized' ), $meta['last_name'] ); // phpcs:ignore WordPress.WP.I18n.NoEmptyStrings
		} elseif ( ! empty( $meta['first_name'] ) && ! empty( $meta['last_name'] ) ) {
			/* translators: 1: last name 2: last name */
			$full_name = sprintf( _x( '%1$s %2$s', 'owb-full-name', 'woocommerce-germanized' ), $meta['first_name'], $meta['last_name'] );
		}
...
function eu_owb_create_order_withdrawal_request( $order, $email, $items = array(), $as_guest = true, $meta = array() ) {
	...
	$item_desc          = array();
	$order_note         = _x( 'A new withdrawal request has been submitted to this order', 'owb', 'woocommerce-germanized' );
	...
	if ( ! $is_full_withdrawal ) {
		$order_note = _x( 'A new partial withdrawal request has been submitted to this order', 'owb', 'woocommerce-germanized' );
	}
...
function eu_owb_order_delete_withdrawal_request( $order, $by_customer = false ) {
	...
	if ( $by_customer ) {
		$order->add_order_note( _x( 'Deleted withdrawal request as requested by the customer.', 'owb', 'woocommerce-germanized' ) );
	} else {
		$order->add_order_note( _x( 'Deleted withdrawal request.', 'owb', 'woocommerce-germanized' ) );
	}
