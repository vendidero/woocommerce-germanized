...
class Package {
							...
							foreach ( $withdrawals as $withdrawal ) :
								...
								<li class="withdrawal withdrawal-<?php echo esc_attr( $withdrawal['status'] ); ?>">
									<div class="withdrawal-content">
										<p><?php echo wp_kses_post( sprintf( _x( '%1$s received on %2$s @ %3$s by <a href="mailto:%3$s">%4$s</a>', 'owb', 'woocommerce-germanized' ), ( 'yes' === $withdrawal['is_partial'] ) ? esc_html_x( 'Partial withdrawal', 'owb', 'woocommerce-germanized' ) : esc_html_x( 'Full withdrawal', 'owb', 'woocommerce-germanized' ), wc_format_datetime( eu_owb_get_order_withdrawal_date_received( $order, $withdrawal ) ), wc_format_datetime( eu_owb_get_order_withdrawal_date_received( $order, $withdrawal ), get_option( 'time_format' ) ), eu_owb_get_order_withdrawal_email( $order, $withdrawal ) ) ); ?></p>
	...
	public static function order_withdrawal_request_details( $order ) {
		...
		<div class="eu-owb-order-withdrawal-request">
			...
			<p><?php echo wp_kses_post( sprintf( _x( 'Received on %1$s @ %2$s by <a href="mailto:%3$s">%3$s</a>', 'owb', 'woocommerce-germanized' ), wc_format_datetime( eu_owb_get_order_withdrawal_date_received( $order, $request ) ), wc_format_datetime( eu_owb_get_order_withdrawal_date_received( $order, $request ), get_option( 'time_format' ) ), eu_owb_get_order_withdrawal_email( $order, $request ) ) ); ?></p>
