	...
	public function get_contract_identification( $context = 'view' ) {
		...
		if ( 'view' === $context ) {
			if ( '' === $contract_identification ) {
				...
				$contract_identification = $contract_identification . ' (' . sprintf( _x( 'Order #%s', 'owb', 'woocommerce-germanized' ), $this->get_order_number() ) . ')';
			}
		}
	...
	public function get_refundable_items() {
		...
		foreach ( $this->get_items() as $item ) {
			if ( $item->get_parent_id() > 0 && $item->get_quantity_left_to_refund() > 0 ) {
				$qty = $item->get_quantity_left_to_refund();

				$items[ $item->get_parent_id() ] = array(
					'quantity' => $qty,
					'qty'      => $qty, // wc_restock_refunded_items support
					'item_id'  => $item->get_id(),
					'item'     => $item,
				);
			}
	...
	public function save() {
		...
		if ( ! $is_new ) {
			if ( array_key_exists( 'parent_id', $changes ) ) {
				$this->add_order_note( sprintf( _x( 'Parent order changed from %1$s to %2$s.', 'owb', 'woocommerce-germanized' ), $original_data['parent_id'], $this->get_parent_id() ) );
			}
		}
