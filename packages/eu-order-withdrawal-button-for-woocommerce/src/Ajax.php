	...
	public static function order_withdrawal_request() {
		...
		if ( ! $is_guest_request && ! empty( $order_id ) ) {
			...
			if ( Package::get_form_field_required( 'order_number' ) && empty( $order_number ) ) {
				$error->add( 'missing_field_order_number', _x( 'Please provide information to identify the contract.', 'owb', 'woocommerce-germanized' ), array( 'field' => 'order_number' ) );
			}
			...
			$validation = eu_owb_get_guest_order_validation_option();

			if ( 'none' !== $validation ) {
				switch ( $validation ) {
					case 'order_existence':
						if ( empty( $order ) ) {
							$error->add( 'not_found', sprintf( _x( 'Sorry, we were unable to find an order based on the information you provided. Please try again - if the issue persists, please <a href="%s">contact support</a> for help.', 'owb', 'woocommerce-germanized' ), esc_url( eu_owb_get_contact_support_url() ) ), array( 'field' => 'order_number' ) );
							self::send_json_error( $error, 404 );
						}
						break;
					case 'order_email_match':
						if ( empty( $order ) || ! eu_owb_custom_email_matches_order_email( $order, $email ) ) {
							$error->add( 'not_found', sprintf( _x( 'Sorry, we were unable to find an order based on the information you provided. Please try again - if the issue persists, please <a href="%s">contact support</a> for help.', 'owb', 'woocommerce-germanized' ), esc_url( eu_owb_get_contact_support_url() ) ), array( 'field' => 'order_number' ) );
							self::send_json_error( $error, 404 );
						}
						break;
					case 'order_or_email':
						if ( empty( $order ) && empty( $meta['email_has_orders'] ) ) {
							$error->add( 'not_found', sprintf( _x( 'Sorry, we were unable to find an order based on the information you provided. Please try again - if the issue persists, please <a href="%s">contact support</a> for help.', 'owb', 'woocommerce-germanized' ), esc_url( eu_owb_get_contact_support_url() ) ), array( 'field' => 'order_number' ) );
							self::send_json_error( $error, 404 );
						}
						break;
