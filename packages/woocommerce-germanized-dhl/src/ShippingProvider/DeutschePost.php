...
class DeutschePost extends Auto {
	...
	protected function get_config_set_simple_label_settings() {
		...
		if ( ! empty( $settings ) ) {
			...
			$settings[0]['desc'] = '<div class="wc-gzd-additional-desc">' . sprintf( _x( 'Manually <a href="%s">refresh</a> the product list to make sure it is up-to-date.', 'dhl', 'woocommerce-germanized' ), esc_url( wp_nonce_url( add_query_arg( array( 'action' => 'wc-gzd-dhl-im-product-refresh' ), $settings_url ), 'wc-gzd-dhl-refresh-im-products' ) ) ) . '</div>';
		}
