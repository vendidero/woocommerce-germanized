...
class DeutschePost extends Auto {
	...
	protected function get_connection_status_html( $maybe_error ) {
		return '<span class="wc-gzd-shipment-api-connection-status ' . ( is_wp_error( $maybe_error ) ? 'connection-status-error' : 'connection-status-success' ) . '">' . ( sprintf( _x( 'Status: %1$s', 'dhl', 'woocommerce-germanized' ), ( is_wp_error( $maybe_error ) ? $maybe_error->get_error_message() : _x( 'Connected', 'dhl', 'woocommerce-germanized' ) ) ) ) . '</span>';
	}
					...
					$settings = array_merge(
						...
						array(
							array(
								...
								'desc'  => '<a class="button button-secondary" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'action' => 'wc-gzd-dhl-im-product-refresh' ), $settings_url ), 'wc-gzd-dhl-refresh-im-products' ) ) . '">' . esc_html_x( 'Refresh available products', 'dhl', 'woocommerce-germanized' ) . '</a>',
							),
							array(
								'type' => 'sectionend',
								'id'   => 'deutsche_post_product_refresh_options',
	...
	protected function get_pickup_settings( $for_shipping_method = false ) {
		$settings = array(
			...
			array(
				...
				'desc_tip'          => _x( 'Limit the number of DHL locations presented to the customer.', 'dhl', 'woocommerce-germanized' ),
				'default'           => 20,
