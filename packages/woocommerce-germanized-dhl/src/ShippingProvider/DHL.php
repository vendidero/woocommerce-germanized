...
class DHL extends Auto {
	...
	protected function get_general_settings( $for_shipping_method = false ) {
		...
		$dhl_available_products = wc_gzd_dhl_get_products_domestic() + wc_gzd_dhl_get_products_eu() + wc_gzd_dhl_get_products_international() + array( 'return' => _x( 'Inlay Returns', 'dhl', 'woocommerce-germanized' ) );
		$dhl_products           = array();
	...
	protected function get_label_settings( $for_shipping_method = false ) {
		...
		$settings = array_merge(
			...
			array(
				...
				array(
					'title'   => _x( 'Recipient signature', 'dhl', 'woocommerce-germanized' ),
					'desc'    => _x( 'Let recipients sign delivery instead of DHL driver.', 'dhl', 'woocommerce-germanized' ),
				...
				array(
					'title'    => _x( 'Endorsement', 'dhl', 'woocommerce-germanized' ),
					'type'     => 'select',
					'default'  => 'return',
					'id'       => 'label_service_Endorsement',
					'value'    => $this->get_setting( 'label_service_Endorsement', 'return' ),
					'desc'     => _x( 'Select how DHL should handle international shipments that could not be delivered.', 'dhl', 'woocommerce-germanized' ),
					'desc_tip' => true,
					'options'  => wc_gzd_dhl_get_endorsement_types(),
					'class'    => 'wc-enhanced-select',
				),
