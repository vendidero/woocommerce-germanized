...
class Paket {
	...
	public function get_label_api() {
		...
		if ( is_null( $this->label_api ) ) {
			if ( ! empty( $error_message ) ) {
				throw new Exception( esc_html( sprintf( _x( 'Label API not available: %s', 'dhl', 'woocommerce-germanized' ), $error_message ) ) );
			} else {
				throw new Exception( esc_html_x( 'Label API not available', 'dhl', 'woocommerce-germanized' ) );
			}
		}
	...
	public function get_finder_api() {
		...
		if ( is_null( $this->finder_api ) ) {
			throw new Exception( esc_html_x( 'Parcel Finder API not available', 'dhl', 'woocommerce-germanized' ) );
		}
	...
	public function get_return_api() {
		...
		if ( is_null( $this->return_api ) ) {
			throw new Exception( esc_html_x( 'Return API not available', 'dhl', 'woocommerce-germanized' ) );
		}
	...
	public function get_parcel_api() {
		...
		if ( is_null( $this->parcel_api ) ) {
			throw new Exception( esc_html_x( 'Parcel API not available', 'dhl', 'woocommerce-germanized' ) );
		}
