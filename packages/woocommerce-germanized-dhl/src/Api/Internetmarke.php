...
class Internetmarke {
	...
	protected function create_or_update_wp_int_label( &$label ) {
		...
		if ( $today >= $discontinued_starting_from ) {
			throw new \Exception( wp_kses_post( sprintf( _x( 'The Deutsche Post WP International API was discontinued. Please use the <a href="%s">DHL API</a> for Warenpost labels instead.', 'dhl', 'woocommerce-germanized' ), esc_url( 'https://vendidero.de/dokument/dhl-integration-einrichten' ) ) ) );
