	...
	protected function handle_get_response( $response_code, $response_body ) {
		...
		switch ( $response_code ) {
			...
			case 400:
				...
				throw new Exception( esc_html( _x( '400 - ', 'dhl', 'woocommerce-germanized' ) . $error_message ), esc_html( $response_code ) );
			case 401:
				throw new Exception( esc_html_x( '401 - Unauthorized Access - Invalid token or Authentication Header parameter', 'dhl', 'woocommerce-germanized' ), esc_html( $response_code ) );
			case 408:
				throw new Exception( esc_html_x( '408 - Request Timeout', 'dhl', 'woocommerce-germanized' ), esc_html( $response_code ) );
			case 429:
				throw new Exception( esc_html_x( '429 - Too many requests in given amount of time', 'dhl', 'woocommerce-germanized' ), esc_html( $response_code ) );
			case 503:
				throw new Exception( esc_html_x( '503 - Service Unavailable', 'dhl', 'woocommerce-germanized' ), esc_html( $response_code ) );
			default:
