...
class ImWarenpostIntRest extends Rest {
	...
	public function update_label( &$label, $result ) {
		...
		if ( ! $pdf ) {
			throw new Exception( esc_html_x( 'Error while fetching label PDF', 'dhl', 'woocommerce-germanized' ) );
		}
	...
	public function create_label( &$label ) {
		...
		if ( ! $shipment = $label->get_shipment() ) {
			throw new Exception( esc_html_x( 'Missing shipment', 'dhl', 'woocommerce-germanized' ) );
		}
		...
		if ( isset( $result->shipments ) ) {
			...
			throw new Exception( esc_html_x( 'Invalid API response', 'dhl', 'woocommerce-germanized' ) );
		}
	}
	...
	protected function get_user_token() {
		...
		if ( ! $user_token ) {
			throw new Exception( esc_html_x( 'Error while authenticating user.', 'dhl', 'woocommerce-germanized' ) );
		}
	...
	protected function handle_get_response( $response_code, $response_body ) {
		switch ( $response_code ) {
			...
			case '201':
				break;
			default:
				throw new Exception( esc_html_x( 'Error during Warenpost International request.', 'dhl', 'woocommerce-germanized' ) );
		}
	}
	...
	protected function handle_post_response( $response_code, $response_body ) {
		switch ( $response_code ) {
			...
			default:
				...
				throw new Exception( wp_kses_post( sprintf( _x( 'Error during request: %s', 'dhl', 'woocommerce-germanized' ), $error_message ) ) );
		}
	}
