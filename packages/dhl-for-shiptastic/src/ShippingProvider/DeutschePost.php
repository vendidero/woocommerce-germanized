...
class DeutschePost extends Auto {
	...
	protected function get_connection_status_html( $maybe_error ) {
		return '<span class="wc-stc-shipment-api-connection-status ' . ( is_wp_error( $maybe_error ) ? 'connection-status-error' : 'connection-status-success' ) . '">' . ( sprintf( _x( 'Status: %1$s', 'dhl', 'woocommerce-germanized' ), ( is_wp_error( $maybe_error ) ? $maybe_error->get_error_message() : _x( 'Connected', 'dhl', 'woocommerce-germanized' ) ) ) ) . '</span>';
	}
