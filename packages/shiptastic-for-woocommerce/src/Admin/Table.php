...
class Table extends WP_List_Table {
	...
	protected function shipping_provider_filter() {
		...
		?>
		<select class="wc-stc-shipping-provider-search" name="shipping_provider" data-placeholder="<?php echo esc_attr_x( 'Filter by shipping service provider', 'shipments', 'woocommerce-germanized' ); ?>" data-allow_clear="true">
	...
	protected function order_filter() {
		...
		?>
		<select class="wc-stc-order-search" name="order_id" data-placeholder="<?php echo esc_attr_x( 'Filter by order', 'shipments', 'woocommerce-germanized' ); ?>" data-allow_clear="true">
	...
	public function column_title( $shipment ) {
		$title = sprintf( _x( '%1$s #%2$s', 'shipment title', 'woocommerce-germanized' ), wc_stc_get_shipment_label_title( $shipment->get_type() ), $shipment->get_shipment_number() );
		...
		echo '<a href="#" class="shipment-preview has-shipment-modal" data-id="wc-stc-modal-preview-shipment" data-load-async="1" data-reference="' . esc_attr( $shipment->get_id() ) . '" data-nonce-params="wc_shiptastic_admin_shipments_table_params">' . esc_html_x( 'Preview', 'shipments', 'woocommerce-germanized' ) . '</a>';
				...
				<div class="wc-backbone-modal-content">
					<section class="wc-backbone-modal-main" role="main">
						<header class="wc-backbone-modal-header">
							<h1><?php printf( esc_html_x( '%1$s %2$s', 'shipments-preview-title', 'woocommerce-germanized' ), esc_html( wc_stc_get_shipment_label_title( $shipment->get_type() ) ), esc_html( $shipment->get_shipment_number() ) ); ?></h1>
						...
						<footer>
							<div class="inner">
								<a class="button button-primary button-large" href="<?php echo esc_url( $shipment->get_edit_shipment_url() ); ?>"><?php echo esc_html_x( 'Edit', 'shipments', 'woocommerce-germanized' ); ?></a>
							</div>
						</footer>
					</section>
				</div>
		...
		if ( ! empty( $provider ) ) {
			echo '<span class="shipment-shipping-provider">' . sprintf( esc_html_x( 'via %s', 'shipments', 'woocommerce-germanized' ), wp_kses_post( wc_stc_get_shipping_provider_title( $provider ) ) ) . '</span> ';
		}
	...
	public function column_date( $shipment ) {
		...
		if ( $shipment_timestamp > strtotime( '-1 day', time() ) && $shipment_timestamp <= time() ) {
			 ...
			 * @package Vendidero/Shiptastic
			 */
			$show_date = $shipment->get_date_created()->date_i18n( apply_filters( 'woocommerce_shiptastic_admin_shipment_date_format', _x( 'M j, Y', 'shipments', 'woocommerce-germanized' ) ) );
		}
	...
	protected function get_bulk_actions() {
		...
		$actions['mark_ready-for-shipping'] = _x( 'Change status to ready for shipping', 'shipments', 'woocommerce-germanized' );
