...
class Ajax {
	...
	public static function create_return_shipment_refund_submit() {
		...
		$response_error = array(
			'success' => false,
			'message' => _x( 'There was an error processing the refund.', 'shipments', 'woocommerce-germanized' ),
		);
	...
	public static function calculate_return_costs() {
		...
		if ( ! ( $order = wc_get_order( $order_id ) ) || ( ! wc_stc_customer_can_add_return_shipment( $order_id ) ) ) {
			$error->add( 'return_not_allowed', _x( 'You are not allowed to add returns to that order.', 'shipments', 'woocommerce-germanized' ) );

			wp_send_json_error( $error, 500 );
		}

		if ( ! wc_stc_order_is_customer_returnable( $order ) ) {
			$error->add( 'return_not_allowed', _x( 'Sorry, but this order does not support returns any longer.', 'shipments', 'woocommerce-germanized' ) );

			wp_send_json_error( $error, 500 );
		}

		if ( empty( $items ) ) {
			$error->add( 'return_empty', _x( 'Please choose one or more items from the list.', 'shipments', 'woocommerce-germanized' ) );
