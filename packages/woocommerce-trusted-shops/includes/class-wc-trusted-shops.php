...
class WC_Trusted_Shops {
	...
	public function get_rich_snippets_locations() {
		...
		if ( 'yes' === $this->rich_snippets_category ) {
			$locations[] = 'category';
		}

		if ( 'yes' === $this->rich_snippets_product ) {
			$locations[] = 'product';
		}

		if ( 'yes' === $this->rich_snippets_home ) {
			$locations[] = 'home';
		}
	...
	public function is_trustbadge_enabled() {
		return ( 'yes' === $this->trustbadge_enable && '' !== $this->id ? true : false );
	}
	...
	public function is_rich_snippets_enabled() {
		return ( 'yes' === $this->rich_snippets_enable && $this->is_enabled() ? true : false );
	}
	...
	public function is_review_widget_enabled() {
		return ( 'yes' === $this->review_widget_enable && $this->is_enabled() ? true : false );
	}

	public function is_review_reminder_enabled() {
		return ( 'yes' === $this->review_reminder_enable && $this->supports( 'reminder' ) && $this->is_enabled() ? true : false );
	}

	public function is_review_reminder_checkbox_enabled() {
		return ( 'yes' === $this->review_reminder_checkbox && $this->is_review_reminder_enabled() ? true : false );
	}
	...
	public function is_product_reviews_enabled() {
		return ( 'yes' === $this->reviews_enable && $this->is_enabled() ? true : false );
	}

	public function is_product_sticker_enabled() {
		return ( $this->is_product_reviews_enabled() && 'yes' === $this->product_sticker_enable ? true : false );
	}

	public function is_review_sticker_enabled() {
		return 'yes' === $this->review_sticker_enable ? true : false;
	}

	public function is_product_widget_enabled() {
		return ( $this->is_product_reviews_enabled() && 'yes' === $this->product_widget_enable ? true : false );
	}

	public function supports( $type ) {
		return ( in_array( $type, $this->supports, true ) ? true : false );
	}
	...
	public function get_new_review_link( $email, $order_id ) {
		return 'https://www.trustedshops.de/bewertung/bewerten_' . $this->id . '.html&buyerEmail=' . rawurlencode( base64_encode( $email ) ) . '&shopOrderID=' . rawurlencode( base64_encode( $order_id ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
	}
	...
	public function get_template( $name ) {
		...
		/**
		 * Use strip_tags instead of wp_strip_all_tags because we want to keep JS.
		 */
		return preg_replace( '/^\h*\v+/m', '', strip_tags( $html ) ); // phpcs:ignore WordPress.WP.AlternativeFunctions.strip_tags_strip_tags
	}

	public function get_script( $name, $replace = true, $args = array() ) {
		$script = $this->get_template( $name );

		if ( 'expert' === $this->integration_mode ) {
		...
		if ( $replace ) {
			$args = wp_parse_args(
				...
				array(
					...
					'variant' => 'standard' === $this->trustbadge_variant ? 'reviews' : 'default',
	...
	public function get_rich_snippets_code( $replace = true, $args = array() ) {
		if ( $replace ) {
			...
			$args = apply_filters(
				'woocommerce_trusted_shops_rich_snippets_args',
				wp_parse_args(
					$args,
					array(
						'average' => $rating['avg'],
