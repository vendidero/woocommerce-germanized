...
class WC_Trusted_Shops_Schedule {
	...
	public function update_default_reviews() {
		...
		if ( $this->base->is_multi_language_setup() ) {
			...
			foreach ( $compatibility->get_languages() as $language ) {
				...
				if ( ! get_option( $option_key, false ) ) {
					$this->update_reviews_inner( $language );
				}
			}
		}
	}
	...
	protected function update_reviews_inner( $lang = '' ) {
		if ( ! empty( $lang ) ) {
	...
	public function send_mails() {
		if ( $this->base->is_multi_language_setup() ) {
			...
			foreach ( $compatibility->get_languages() as $language ) {
				$this->send_mails_inner( $language );
			}
		} else {
			$this->send_mails_inner();
		}
	}

	protected function send_mails_inner( $lang = '' ) {
		if ( ! empty( $lang ) ) {
			wc_ts_switch_language( $lang );
		}
