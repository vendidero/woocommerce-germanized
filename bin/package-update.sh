...
find ./packages/shiptastic-integration-for-dhl -iname '*.php' -exec sed -i.bak -e "s/, 'shiptastic-integration-for-dhl'/, 'woocommerce-germanized'/g" {} \;
...
find ./packages/shiptastic-integration-for-dhl \( -iname '*.js' -o -iname '*.json' \) -exec sed -i.bak -e "s/'shiptastic-integration-for-dhl'/'woocommerce-germanized'/g" -e "s/\"shiptastic-integration-for-dhl\"/\"woocommerce-germanized\"/g" {} \;
...
if [ $COMPOSER_DEV_MODE -eq 0 ]; then
    ...
    rm -f ./packages/shiptastic-integration-for-dhl/i18n/shiptastic-integration-for-dhl.pot
...
rm -rf ./packages/shiptastic-integration-for-dhl/vendor
