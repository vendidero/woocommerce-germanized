 ...
 * ProductPrice class.
 */
class ProductUnitPrice extends AbstractBlock {
	/**
	 * Block namespace.
	 *
	 * @var string
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( $product->has_unit() ) {
			...
			return sprintf(
				'<div class="wp-block-woocommerce-product-price"><div class="wc-block-components-product-price wc-block-grid__product-price %1$s %2$s" style="%3$s"><p class="price-unit"
					%4$s
				</p></div></div>',
				esc_attr( $text_align_styles_and_classes['class'] ?? '' ),
				esc_attr( $styles_and_classes['classes'] ),
				esc_attr( $styles_and_classes['styles'] ?? '' ),
				$product->get_unit_price_html()
			);
		}
	}
}
