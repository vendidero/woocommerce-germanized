...
<div id="message" class="error woocommerce-gzd-message wc-connect">
	...
	<p>
		...
		elseif ( $dependencies->is_woocommerce_outdated() ) :
			...
			echo wp_kses_post( sprintf( __( '%1$sGermanized is inactive.%2$s This version of Germanized requires WooCommerce %3$s or newer. Please %4$supdate WooCommerce to version %3$s or newer &raquo;%5$s', 'woocommerce-germanized' ), '<strong>', '</strong>', $dependencies->get_wc_min_version_required(), '<a href="' . esc_url( admin_url( 'plugins.php' ) ) . '">', '</a>' ) );
		endif;
		?>
	</p>
</div>
