	...
	foreach ( WC_germanized()->compatibilities as $c => $comp ) :
		...
		<tr>
			...
			<td>
				...
				<?php echo( ! $comp->is_supported() ? sprintf( __( 'Version %1$s not supported, supporting version %2$s - %3$s', 'woocommerce-germanized' ), $version_data['version'], $version_data['requires_at_least'], $version_data['tested_up_to'] ) : '' ); ?>
			</td>
		</tr>
				...
				<?php if ( ! empty( $data['files'] ) ) : ?>
					<?php foreach ( $data['files'] as $file ) : ?>
						...
						<?php if ( $file['outdated'] ) : ?>
							<?php printf( __( 'Version %1$s is out of date. The core version is %2$s.', 'woocommerce-germanized' ), '<span class="red" style="color:red">' . $file['theme_version'] . '</span>', $file['core_version'] ); ?>
						<?php endif; ?>
