...
<table class="wc_status_table widefat" cellspacing="0" id="status">
	...
	<tbody>
		<?php foreach( WC_GZD_Admin::instance()->get_template_version_check_result() as $plugin => $data ) : ?>
            <tr>
                <td data-export-label="Overrides"><?php _e( 'Overrides', 'woocommerce-germanized' ); ?>:<br/><strong><?php echo esc_html( $data['title'] ); ?></strong></td>
                ...
                <td>
                    <?php if ( ! empty( $data['files'] ) ) : ?>
                        ...
                        <?php if ( $data['has_outdated'] ) : ?>
                            <br/><a href="<?php echo esc_url( $data['outdated_help_url'] ); ?>" target="_blank"><?php _e( 'Learn how to update outdated templates', 'woocommerce-germanized' ) ?></a>
                        <?php endif; ?>
                    <?php else: ?>
                        &ndash;
                    <?php endif; ?>
