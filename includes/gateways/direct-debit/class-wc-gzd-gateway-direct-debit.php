...
Please notice: Period for pre-information of the SEPA direct debit is shortened to one day.', 'woocommerce-germanized' ) );
    ...
    public function set_order_meta( $order_id ) {
    	...
    	$check_holder = strcmp($user_meta['holder'], $post_holder);
    	$check_iban = strcmp($user_meta['iban'], $post_iban);
    	$check_bic = strcmp($user_meta['bic'], $post_bic);

    	if ( $check_holder == 0 && $check_iban == 0 && $check_bic == 0 ) {
    		// Keep existing mandate
    	} else {
    		$this->create_recurring_mandate_for_user( $order );
    	}
    ...
    public function create_recurring_mandate_for_user( $subscription_order ) {
	...
	public function generate_mandate() {
    	...
    	if ( wcs_order_contains_subscription( $post->ID, array( 'parent', 'renewal' ) ) || WC_Subscriptions_Cart::cart_contains_subscription() || get_user_meta( get_current_user_id(), 'direct_debit_mandate_id', true) || $create_user_checked ) {
    		$mandate_type_text = __('recurring payments','woocommerce-germanized');
    	} else {
    ...
    public function generate_mandate_by_order( $order ) {
    	...
    	if ( wcs_order_contains_subscription( $order->ID, array( 'parent', 'renewal' ) ) || get_user_meta(wc_gzd_get_crud_data( $order, 'user_id' ), 'direct_debit_mandate_id', true)) {
    		$mandate_type_text = __('recurring payments','woocommerce-germanized');
    	} else {
    		$mandate_type_text = __('a single payment','woocommerce-germanized');
    	}
