...
class WC_GZD_Compatibility_Cartflows extends WC_GZD_Compatibility {
	...
	public function load() {
		 ...
		 * AJAX action support
		 */
		add_action( 'cartflows_woo_checkout_update_order_review', function( $post_data ) {
		 ...
		 * Checkout support
		 */
		add_action( 'cartflows_checkout_form_before', function( $checkout_id ) {
			if ( ! $this->enable_cartflows_support() ) {
				return;
			}
		 ...
		 * Opt-in support
		 */
		add_action( 'cartflows_optin_form_before', function( $optin_id ) {
            if ( ! $this->enable_cartflows_support() ) {
                return;
            }
